  ...
  return (
    <header className="border-b border-black/15 bg-[#202821] text-[#f0ebde] dark:border-white/10 dark:bg-[#111512]">
      <div className="mx-auto flex max-w-[120rem] items-center justify-between px-4 py-3 sm:px-6 lg:px-[24px] lg:py-[12px]">
        ...
        <div className="grid grid-cols-2 items-center gap-1 sm:flex sm:gap-3 lg:gap-[12px]">
          <div className="contents sm:flex sm:items-center sm:gap-1 lg:gap-[4px]"><AudioController /><ThemeToggle /></div>
          <div className="relative">
            <Button variant="ghost" size="sm" className="size-8 rounded-sm px-0 text-[#aeb8ad] hover:bg-white/10 hover:text-white sm:h-8 sm:w-auto sm:px-3 lg:gap-[4px] lg:px-[12px] lg:text-[14px]" onClick={() => { setObjectiveOpen((open) => !open); setSystemsOpen(false) }} aria-expanded={objectiveOpen} aria-controls="journey-objective-menu"><Flag className="size-4 lg:size-[16px]" /><span className="hidden sm:inline">Objective</span><ChevronDown className={`hidden size-3 transition-transform sm:block lg:size-[12px] ${objectiveOpen ? "rotate-180" : ""}`} /></Button>
            {objectiveOpen && <div id="journey-objective-menu" className="fixed top-[5.75rem] right-4 left-4 z-50 max-h-[calc(100svh-6.25rem)] overflow-y-auto border border-white/15 bg-[#202521] text-[#ebe5d7] shadow-2xl sm:absolute sm:top-[calc(100%+0.75rem)] sm:right-0 sm:left-auto sm:max-h-none sm:w-[calc(100vw-2rem)] sm:overflow-visible lg:top-[44px] lg:w-[384px]"><JourneyObjective embedded /></div>}
          </div>
          <div className="relative">
            <Button variant="ghost" size="sm" className="size-8 rounded-sm px-0 text-[#aeb8ad] hover:bg-white/10 hover:text-white sm:h-8 sm:w-auto sm:px-3 lg:gap-[4px] lg:px-[12px] lg:text-[14px]" onClick={() => { setSystemsOpen((open) => !open); setObjectiveOpen(false) }} aria-expanded={systemsOpen} aria-controls="convoy-systems-menu"><SlidersHorizontal className="size-4 lg:size-[16px]" /><span className="hidden sm:inline">Systems</span><ChevronDown className={`hidden size-3 transition-transform sm:block lg:size-[12px] ${systemsOpen ? "rotate-180" : ""}`} /></Button>
            {systemsOpen && <div id="convoy-systems-menu" className="fixed top-[5.75rem] right-4 left-4 z-50 max-h-[calc(100svh-6.25rem)] overflow-y-auto divide-y divide-white/10 border border-white/15 bg-[#202521] text-[#ebe5d7] shadow-2xl sm:absolute sm:top-[calc(100%+0.75rem)] sm:right-0 sm:left-auto sm:w-[calc(100vw-2rem)] lg:top-[44px] lg:w-[400px]"><FuelPanel /><TaskPanel /><UpgradePanel /><InteriorPanel /></div>}
          </div>
        </div>
      </div>
    </header>
